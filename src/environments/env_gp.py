...
class EnvGP():
    ...
    def get_next_action(self, priority_func: Any):
        ...
        #priority_funcs_no = len(priority_funcs)
        eval_assemble = True if isinstance(priority_func, list) else False

        #print("get_next_action", type(priority_func))#!!!!!!!!!!!!!aici de testat daca e lista sau nu
        ready_pairs = []
                    ...
                    if eligible_machine == 1:
                        ...
                        if self.evaluation_type == 'assemble':
                            for pf in priority_func:
                                a= pf(feat_op_mean_time, feat_op_min_time, feat_op_flexibility_factor,
                                                      feat_machine_ready_time, feat_machine_operation_proportion,
                                                      feat_machine_utilization_percentage,
    ...
    def step(self, action, **args):
        ...
        if selected_op_parent_idx is not None:
            ...
            if parent_ready:
                ...
                selected_op_parent.release_time = completion_time
    ...
    def evaluate_instance(self, priority_funcs: Any)->float:
            """
            ...
            while not self.done:
                action = self.get_next_action(priority_funcs)
                self.step(action)
