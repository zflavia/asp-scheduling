...
class GP_Two_Trees(GPBase):
    ...
    def str_individual(self, individual):
        return f"{infix_str(individual[0])}; {infix_str(individual[1])}"

    def display_hof(self, hof, toolbox):
        best_ind = hof[0]
        print(f"Best individual: {self.str_individual(best_ind)}")
        print(f" operation selection: {infix_str(best_ind[0])}")
        print(f" machine selection:   {infix_str(best_ind[1])}")

        simp = simplify_individual(best_ind[0], toolbox.pset_dispatch)
        print(f"  Simplified operation selection: {infix_str(simp)}")
        simp = simplify_individual(best_ind[1], toolbox.pset_route)
        print(f"  Simplified machine selection:   {infix_str(simp)}")

        for indx, ind in enumerate(hof):
            print(f"HOF {indx}: {self.str_individual(ind)}")
            simp = simplify_individual(ind[0], toolbox.pset_dispatch)
            print(f"  Simplified operation selection: {infix_str(simp)}")
            simp = simplify_individual(ind[1], toolbox.pset_route)
            print(f"  Simplified machine selection:   {infix_str(simp)}")
    ...
    def register_individual_statistic(self, tools):
        ...
        stats_best_ind_obj.register("operation_selection_rule", disp_expr)
        stats_best_ind_obj.register("machine_selection_rule", route_expr)
