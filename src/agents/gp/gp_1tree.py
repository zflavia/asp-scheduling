...
class GP_One_Tree(GPBase):
    ...
    def configure_terminals(cls):
        ...
        :return: pset
        """
        pset = gp.PrimitiveSetTyped("GP_pair", [float] * 5, float)
        ...
        # pset = gp.PrimitiveSetTyped("GP_pair", [float]*16, float)
    ...
    def str_individual(self, individual):
        return infix_str(individual)

    def display_hof(self, hof, toolbox):
        print(f"Best individual: {self.str_individual(hof[0])}")

        simp = simplify_individual(hof[0], toolbox.pset)
        print(f"  Simplified: {self.str_individual(simp)}")
    ...
    def register_individual_statistic(self, tools):
        ...
        stats_best.register("best_tree", lambda pop: str(min(pop, key=lambda i: i.fitness.values[0])))
